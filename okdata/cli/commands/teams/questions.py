...
def q_team(team_client, my=False):
    def _team_choices():
...
def q_attribute(team_client):
    def _attribute_choices(team_id):
        ...
        name = team["name"]
        email = team["attributes"].get("email", [None])[0]
        slack_url = team["attributes"].get("slack-url", [None])[0]
...
def q_members(team_client):
    def _team_member_choices(team_id):
        ...
        return [
            ...
            for m in sorted(
                members,
                # Sort by name and username. Users without name are sorted to the end.
                key=lambda x: (x["name"] is None, x["name"], x["username"]),
            )
        ]
