  ...
  --history
    ...
    def handler(self):
        ...
        if self.cmd("ls"):
            ...
            fmt_args = [scope, resource_name, user_class.__name__.lower(), user]
            if self.cmd("add"):
                self.add_user(resource_name, scope, user_class(user))
                self.print("Granted '{}' on '{}' to {} '{}'".format(*fmt_args))
            else:
                self.remove_user(resource_name, scope, user_class(user))
    ...
    def add_user(self, resource_name, scope, user):
        """Grant `scope` on `resource_name` to `user`."""
        self.client.update_permission(resource_name, scope, add_users=[user])

    def remove_user(self, resource_name, scope, user):
        """Revoke `scope` on `resource_name` from `user`."""
